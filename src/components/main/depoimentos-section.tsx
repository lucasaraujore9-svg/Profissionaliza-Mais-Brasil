...
const featured: Depoimento | null = null

const secundarios: Depoimento[] = []

export function DepoimentosSection() {
  const temDepoimentos = featured !== null || secundarios.length > 0
  ...
  return (
    <section className="bg-white py-20 md:py-28">
      <div className="mx-auto max-w-6xl px-4 md:px-8">
        ...
        {!temDepoimentos ? (
          <div
            className="mt-14 flex flex-col items-center justify-center rounded-3xl border border-dashed border-[var(--color-pmb-green-900)]/15 bg-[var(--color-pmb-mist)] px-6 py-14 text-center"
            data-reveal
            data-reveal-delay="0.1"
          >
            <Quote
              aria-hidden
              className="h-10 w-10 text-[var(--color-pmb-green)] opacity-70"
              strokeWidth={1.75}
            />
            <p className="mt-5 max-w-xl text-lg font-bold text-[var(--color-pmb-green-900)]">
              Em breve, depoimentos de quem já faz parte da nossa rede.
            </p>
            <p className="mt-2 max-w-md text-sm leading-relaxed text-gray-600">
              Estamos coletando relatos reais de parceiros para compartilhar aqui.
            </p>
          </div>
        ) : (
