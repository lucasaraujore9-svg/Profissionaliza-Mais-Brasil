...
export default async function AdminAtendimentoPage({
  ...
  const rows = await prisma.contactMessage.findMany({
    where: {
      tenantId: null, // caixa da PMB institucional
      status,
      ...(kind !== "ALL" ? { kind } : {}),
    },
  ...
  function tabHref(next: { status?: Status; kind?: KindFilter }) {
    ...
    return `/admin/atendimento?${p.toString()}`
  }

  return (
    <div className="space-y-6">
      <PageHeader
        title="Atendimento · Vitrine PMB"
        description="Mensagens de contato e chamados de suporte dos alunos da vitrine institucional."
      />
