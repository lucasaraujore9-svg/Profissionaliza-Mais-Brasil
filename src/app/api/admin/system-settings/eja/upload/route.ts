...
// SVGs sao bloqueados deliberadamente (podem carregar <script>).
...
// Upload do banner da seção "EJA". A imagem é uma config system-wide (definida
// pela PMB e herdada por toda a rede), por isso o guard é SUPER_ADMIN. Vai para
// o bucket `vitrine-assets` sob o prefixo `eja/`; a URL pública servida de
// `*.supabase.co` já está liberada no CSP.
export const POST = withRequestContext(
  {
    action: "admin.system_settings.eja.upload",
    route: "/api/admin/system-settings/eja/upload",
  },
  async (request: Request) => {
    ...
    const path = `eja/${Date.now()}-${Math.round(Math.random() * 1e9)}.${ext}`

    try {
      const buffer = await file.arrayBuffer()
      // Magic byte check — mime declarado pelo client é spoofable.
