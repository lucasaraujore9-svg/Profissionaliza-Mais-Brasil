...
- **Severidade:** P2
- **Status:** ✅ Corrigido (2026-06-20) — em `src/app/api/loja/checkout/route.ts`: (a) `prisma.tenantCourse.findFirst` agora filtra `price: { gt: 0 }` (TenantCourse `isVisible:true` + `price:0` ⇒ 404 `COURSE_NOT_FOUND`, mesma invariante das listagens); (b) trava explícita após `basePrice` via novo helper puro `isSellablePrice` (`src/lib/checkout/price-guard.ts`) ⇒ 400 `COURSE_NO_PRICE` (cobre Decimal não-positivo / NaN). Teste: `src/lib/checkout/price-guard.test.ts` (6 casos: positivo OK; zero/negativo/NaN/Infinity/null-undefined rejeitados). Os checkouts de pacote (`finalAmount: 0` intencional) seguem por outro caminho, não afetados. Portão: tsc/lint/test (139) verdes; build delegado ao CI.
...
- **Severidade:** P2
- **Status:** ✅ Corrigido (2026-06-20) — política escolhida = **freeze sem over-clawback** (opção b da recomendação). Em `PAYMENT_PARTIALLY_REFUNDED` (`src/lib/asaas/process.ts`) o sistema NÃO cancela/reverte a comissão (valor proporcional é decisão do admin), mas **congela o saque**: nova `freezeCommissionForPartialRefund` (`src/lib/referrals/commission.ts`) marca a comissão legada (qualquer status ativo) com `[CLAWBACK_PENDING]` sem mudar o status; `flagMonthlyCommissionForRefund` faz o mesmo no motor por faixas; e `logAudit({action:"referral.partial_refund.freeze"})` registra. Os **gates de saque** foram ampliados em `src/lib/referrals/payout.ts` (`requestPayout` + `processMonthlyPayouts`) para bloquear sobre **qualquer** comissão legada marcada (antes só `status:"PAID"`), fechando a janela em que o indicador sacava a comissão integral de uma mensalidade parcialmente estornada. Marcador centralizado em `src/lib/referrals/clawback.ts` (`CLAWBACK_MARKER_PREFIX` + `isClawbackMarked`/`hasClawbackBlock`). Sem regressão no refund total (rows não-PAID continuam CANCELLED sem marcador). Teste: `src/lib/referrals/clawback.test.ts` (9 casos: predicados + semântica do gate bloqueando PENDING congelada + contrato do prefixo). Portão: tsc/lint/test (148) verdes; build delegado ao CI.
