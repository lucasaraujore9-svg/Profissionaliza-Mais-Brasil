...
- **Severidade:** P2
- **Status:** ✅ Corrigido — SQL versionado (2026-06-20). Migration aditiva + idempotente `prisma/migrations/20260620_pmb_null_unique_indexes/migration.sql` cria 4 índices ÚNICOS PARCIAIS no escopo PMB (`WHERE tenant_id IS NULL`): `coupons_pmb_code_key`(coupons.code), `course_packages_pmb_slug_key`(course_packages.slug), `automation_templates_pmb_key_key`(automation_message_templates.key) e `certificate_templates_pmb_singleton_key`((true)) — este garante no máximo UM template PMB. `CREATE UNIQUE INDEX IF NOT EXISTS` (re-executável), SEM `CONCURRENTLY` (runner aplica em transação), e cada CREATE envolto em `DO/EXCEPTION WHEN unique_violation` que emite `RAISE WARNING` acionável (não derruba o deploy) caso existam duplicatas PMB pré-existentes — não-destrutivo. **NÃO rodada contra banco nenhum** (sem `DATABASE_URL` local); o `apply-pending-migrations.mjs` a aplica no deploy. Índices parciais não são expressáveis em `@@unique` do Prisma → schema.prisma intacto, por design (documentado). *Follow-up não-bloqueante (fora do escopo desta correção):* trocar os `findFirst`+`create` do seed por `create`+catch P2002 agora que o banco garante a unicidade. Portão: tsc/lint/test (148) verdes; aplicação da migration delegada ao deploy.
