...
- **Severidade:** P2
- **Status:** ✅ Corrigido (2026-06-20) — `xlsx` removido do `package.json`/lockfile e substituído por `exceljs@^4.4.0` (mantido e patchável no npm). `src/components/admin/report-viewer.tsx:exportXlsx()` reescrita com `ExcelJS.Workbook` (escrita-apenas; `aoa_to_sheet`/`XLSX.write` → `addWorksheet`/`addRow`/`xlsx.writeBuffer`), preservando header+linhas, largura de coluna 18, nome de aba truncado a 31 chars (agora também sanitiza `: \ / ? * [ ]`, proibidos pelo Excel) e o mesmo MIME/filename. Sem `XLSX.read` em lugar nenhum (nunca houve parsing de upload). Nota atualizada em `docs/SECURITY.md`. Portão: tsc/lint/test verdes; build delegado ao CI (comando `next build` indisponível no sandbox de execução).
