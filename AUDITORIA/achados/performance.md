...
- **Rodada de correção 2026-07-03 (domínio performance):** P1 **PERF-010** (+API-007), **PERF-002**, **PERF-003** Corrigidos. P2 **PERF-001, PERF-004, PERF-005, PERF-006, PERF-007, PERF-011** Corrigidos. P3 **PERF-012, PERF-013** Corrigidos; **PERF-008** Aceito (ação manual de infra — cota Image Optimization Vercel); **PERF-009** Aceito (bundle-analyzer é webpack, incompatível com o build Turbopack; `output:'standalone'` é migração VPS). Portão Zero-Erro verde ao fim de cada correção (última rodada: 406 testes).
...
- **Verificação:** Novo cache de DADOS cross-request `src/lib/home/cache.ts` (Redis Upstash via `getJson`/`setJson`/`invalidateMany` — já fail-open no padrão dcd03fd, um outage do Redis cai direto no Postgres, nunca derruba a home). Chaves **namespaceadas por tenant**: `home:sections:{tenantId|pmb}` e `home:showcase:{tenantId|pmb}`, TTL 60s. `loadHomeSectionsCached`/`loadShowcaseCached` cobrem as leituras DETERMINÍSTICAS por-escopo que toda visita re-lia do Postgres: a estrutura de seções (`loadHomeSections` em `dynamic-home-sections.tsx`) e o showcase do hero (`loadShowcase` em `(main)/page.tsx` + `loja/page.tsx`). Invalidação explícita `invalidateHomeCache(scope.tenantId)` nos 4 writes de seção (`src/lib/home/api.ts`: create/update/delete/reorder) — cobre admin (PMB, tenantId=null) e painel (revenda). `force-dynamic` das páginas **não** foi alterado (HTML segue dinâmico; só os dados cacheiam). Escopo deliberado: a resolução de cursos por seção (`resolveSectionCourses`) NÃO é cacheada porque o modo "random" sorteia por visita + mantém snapshot por cookie — cacheá-la mudaria o comportamento; o cache reduz carga sem congelar o sorteio. Teste `src/lib/home/cache.test.ts` (8): HIT não toca o Postgres, MISS lê+popula com chave por-tenant, vazio não cacheia, invalidação apaga as 2 chaves do escopo. Portão verde: typecheck 0 · lint 0 erros (1 warn pré-existente) · build exit 0 · test 381 passed. Commit: 57a1da8.
...
- **Severidade:** P1
- **Status:** Corrigido (2026-07-03)
- **Verificação:** Paginação server-side por querystring (`?page=N`, `PAGE_SIZE=24`) + `select` enxuto. `loadCatalogo` ganhou `skip` (já tinha `take`); `listTenantCatalog` ganhou `take`/`skip`. `src/app/(main)/cursos/page.tsx` e `src/app/loja/cursos/page.tsx` lêem `page` dos `searchParams`, passam `take/skip`, calculam `totalPages` do `count` e renderizam `<CatalogPager>` (`src/components/shared/catalog-pager.tsx` — server component, Links prev/próxima preservando `q`/`categoria`, some com 1 página). Branch "sem resultados" passou a carregar só a 1ª página (`take: PAGE_SIZE`) em vez do catálogo inteiro. `listTenantCatalog`/`listTenantCourses`: `include: { course: true }` (toda a linha `Course`) trocado por `include: { course: { select: CARD_COURSE_SELECT } }` — só as 13 colunas que `mapTenantCourseItem` lê (o typecheck garante a cobertura). Layout preservado (grid inalterado; pager adiciona só a navegação). Testes: `src/lib/catalog/loadCatalogo-pagination.test.ts` (take/skip forwardados ao Prisma; total do `count`; skip=0/ausente não injeta offset). Portão verde: typecheck 0 · lint 0 erros (1 warn pré-existente) · build exit 0 · test 384 passed. Commit: d04cc54.
...
- **Status:** Corrigido (2026-07-03)
- **Verificação:** Sync INCREMENTAL por evento. `processLmsWebhookEvent` (`src/lib/webhooks/lms-process.ts`) deixou de chamar `syncCatalogFromLMS("cron")` (varredura de todo o catálogo) nos eventos de catálogo. Agora: `course.published`/`course.updated` → `syncSingleLmsCourse(slug)` (1 pull do detalhe `/courses/:slug` + 1 upsert do curso do evento); `course.unpublished` → `deactivateLmsCourse(courseId)` (`updateMany` por `lmsCourseId`, espelhando o consumidor de `day-update`). O corpo do upsert de curso (`sync-lms.ts`) foi extraído em `upsertLmsCourse`, reusado pelo loop do sync completo e pelo caminho incremental — **mesmas invariantes**: em UPDATE preserva `status`/categoria principal do admin (não reverte curadoria, `sync-lms.ts` ex-linha 195); em CREATE nasce oculto na mãe sem preço>0 E categoria (gate de preço em runtime intacto). Detalhe 404 (curso não publicado) → `null` → 200 sem erro (sem re-tentativa). Testes: `src/lib/webhooks/lms-catalog-webhook.test.ts` (6) — published/novo, updated/curado, 404→ok, unpublished→INATIVO por id, e schemas exigindo slug/courseId; confirma que `syncCatalogFromLMS` completo NUNCA é chamado no caminho de webhook. O cron `sync-cursos-lms` diário segue como backstop de convergência total. Portão verde: typecheck 0 · lint 0 erros (1 warning pré-existente) · build exit 0 · test 373 passed. Commit: 3a02a48.
...
- **Severidade:** P2
- **Status:** Corrigido (2026-07-03)
- **Verificação:** Novo cache de branding `tenant:branding:{id}` (TTL 300s). `getCurrentTenant` (`src/lib/tenant/current.ts`): quando o proxy injeta `x-tenant-id` (caso comum), lê `getTenantBranding<CurrentTenant>(id)` ANTES do `findFirst`; hit retorna sem tocar o Postgres; miss faz o `findFirst` e popula `setTenantBranding(tenant.id, tenant)` (chaveado pelo id real — cobre inclusive o resolve por slug). Fail-open: `getTenantBranding` devolve `null` num outage do Redis → cai no `findFirst`. Helpers `getTenantBranding`/`setTenantBranding` + `tenantBrandingKey` em `src/lib/redis/{tenant-cache,keys}.ts`. Invalidação **central**: a chave de branding foi adicionada a `invalidateTenant` — os ~24 callers existentes (vitrine/banner/domínio/billing/status/eja/técnica/automação) já a limpam, então editar a vitrine no painel reflete no próximo request. Teste `src/lib/tenant/current.test.ts` (4): HIT sem Prisma, MISS lê+popula por id, resolve-por-slug popula pelo id real, sem headers → null. Portão verde: typecheck 0 · lint 0 erros (1 warn pré-existente) · build exit 0 · test 393 passed. Commit: c97c7ff.
...
- **Severidade:** P2
- **Status:** Corrigido (2026-07-03)
- **Verificação:** O `findMany` único sem teto virou 1 query CAPADA por coluna do kanban (`PER_STAGE_LIMIT=200`), em paralelo (`Promise.all(ALL_STAGES.map(...))`), em ambos os handlers (`admin/leads`, `painel/leads`). Cada query filtra por `stage` + `take`, ordenada por `columnOrder`/`createdAt` — preserva a ordenação intra-coluna do drag-and-drop e o shape do board. No painel, o mapa de responsáveis (`listLeadAssignees`) entra no mesmo `Promise.all`. Teste `src/app/api/admin/leads/route.test.ts` (2): 6 queries com `take:200` + board particionado por stage; papel sem permissão → 403 sem consultar leads. Portão verde: typecheck 0 · lint 0 erros (1 warn pré-existente) · build exit 0 · test 389 passed. Commit: afc004b.
...
- **Severidade:** P2
- **Status:** Corrigido (2026-07-03)
- **Verificação:** Os 4 crons ganharam teto por execução com progresso garantido (mantido o IO serial de propósito — rate limit EA/LMS/Asaas). Estratégia por tipo de progresso:
  - **`sweep-students-expired`** (`take: 500` + `orderBy expiresAt asc`): loop de encerramento tem progresso garantido (expirada → CANCELLED sai do filtro); loop de avisos prioriza os mais próximos do fim (a dedup por `accessWarnDaysSent` cobre catch-up dos que sobrarem).
  - **`reactivate-paid`** (`take: 500` ambos os loops): a ação (status → ACTIVE) remove a linha do filtro `SUSPENDED`; no loop de enrollments empurrei o gate `student.tenant.status = ACTIVE` (mesmo do `canReactivateUnderTenant`) para a query, evitando puxar linhas não-acionáveis que ficariam no filtro.
  - **`reconcile-tenant-payments`** e **`sweep-students-overdue`** (cursor keyset por `id` no Redis, `take: 300`/`500`): a ação NÃO remove a linha do filtro, então um `take` fixo starvaria a cauda — o cursor avança a cada execução e zera ao varrer a última página, cobrindo todos ao longo de poucas execuções. Fail-open: Redis off → cursor null → começa do início (idempotente). Cursor via `get`/`set`/`invalidate` de `src/lib/redis/cache.ts`.
  Teste `src/app/api/cron/reconcile-tenant-payments/route.test.ts` (2): cursor lido→injetado no where, lote cheio avança o cursor, lote parcial zera. Portão verde: typecheck 0 · lint 0 erros (1 warn pré-existente) · build exit 0 · test 395 passed. Commit: ce9b199. ⚠️MIGRAÇÃO VPS: o fix definitivo é BullMQ (job por item) — o cap/cursor é a ponte segura até lá.
...
- **Severidade:** P2
- **Status:** Corrigido (2026-07-03)
- **Verificação:** Cursor keyset por `id` (`?afterId=<certId>`) + `take: PAGE_SIZE=200` em `src/app/api/admin/certificates/regenerate-all/route.ts`. Cada request regenera no máximo 200 PDFs (mantido `BATCH=4` de concorrência) e devolve `{ count, regenerated, failed, failures, nextAfterId }`; página cheia → `nextAfterId` = último id, parcial → `null`. O client `src/components/admin/regenerate-all-certificates.tsx` itera as páginas até `nextAfterId=null` acumulando os totais (UX "regenerar todos" segue num clique, mas cada request é limitado — não estoura os 300s; retomada natural). `orderBy: id asc` (chave estável do cursor). Teste `src/app/api/admin/certificates/regenerate-all/route.test.ts` (2): página cheia devolve nextAfterId + take=200; parcial devolve null + respeita `afterId`. Portão verde: typecheck 0 · lint 0 erros (1 warn pré-existente) · build exit 0 · test 397 passed. Commit: 06fb9e4. ⚠️MIGRAÇÃO VPS: BullMQ (1 job por certificado) segue como evolução.
...
- **Severidade:** P2
- **Status:** Corrigido (2026-07-03)
- **Verificação:** Snapshot compartilhado via Redis com TTL = tick (5s). Novos wrappers em `src/lib/placar/snapshot.ts`: `getPlacarSnapshotCached`/`getActiveTenantsCached` (chaves globais `placar:snapshot`/`placar:active`) e `getReferralPlacarSnapshotCached`/`getReferralActiveTenantsCached` (chaves por-tenant `placar:ref:snapshot:{id}`/`placar:ref:active:{id}` — isolamento entre revendedores). Os dois streams (`api/placar/stream`, `api/painel/placar/stream`) passaram a chamar os wrappers (via alias no import — semântica do stream inalterada: baseline por-conexão e detecção de "venda nova" preservados, cada conexão compara o set cacheado atual contra seu próprio baseline). Custo no Postgres cai de `N_conexões × ~4 queries/5s` para `~4 queries/5s por chave`. Fail-open (dcd03fd): `getJson`→null / `setJson` swallow ⇒ cai no DB. Teste `src/lib/placar/snapshot-cache.test.ts` (3): HIT sem Postgres, MISS popula com TTL 5s, chave de indicações namespaceada por `referrerTenantId`. Portão verde: typecheck 0 · lint 0 erros (1 warn pré-existente) · build exit 0 · test 400 passed. Commit: 22b224c. ⚠️MIGRAÇÃO VPS: SSE de longa duração exige timeout do Traefik.
...
- **Severidade:** P3
- **Status:** Aceito (ação manual do usuário — decisão de infra)
- **Nota (2026-07-03):** NÃO corrigível com segurança em código. O flag `unoptimized: true` está ligado por a cota de Image Optimization da Vercel ter estourado (402 `OPTIMIZED_IMAGE_REQUEST_PAYMENT_REQUIRED`, documentado em `next.config.ts:59`). Remover o flag AGORA reintroduziria o 402 e **quebraria** o carregamento de imagens em prod. O fechamento depende de uma decisão/ação de infra do dono: (a) reativar/pagar a Image Optimization na Vercel e então remover o flag; OU (b) ⚠️MIGRAÇÃO VPS: loader próprio (`sharp` com `output:'standalone'`, imgproxy/thumbor ou transformação no MinIO) + atualizar `remotePatterns`. O corretor PARA aqui (regra de ouro 9: decisão de infra/custo).
...
- **Severidade:** P3
- **Status:** Aceito (risco assumido — incompatível/migração)
- **Nota (2026-07-03):** As duas receitas não se aplicam com segurança ao estado atual do repo:
  - **`@next/bundle-analyzer`**: é um plugin **de webpack**; o build do repo roda sob **Turbopack** (Next 16.2.6 — o build log emite "Next.js 16.2.6 (Turbopack)"), então `withBundleAnalyzer` **não produziria o treemap**. Ao testar a adição (`npm i -D @next/bundle-analyzer`), ela puxou **16 pacotes extras** e subiu o count de vulnerabilidades — custo sem benefício sob Turbopack. Revertido. O caminho correto é a análise nativa do Turbopack (ou um analisador compatível), a decidir. Dynamic import das libs pesadas (driver.js, recharts via barrel `dynamic(ssr:false)`, jspdf/gsap) já isola o bundle inicial hoje.
  - **`output: 'standalone'`**: é item de **⚠️MIGRAÇÃO VPS** (necessário para a imagem Docker/Swarm). Ligá-lo agora na Vercel altera o output do build sem benefício em prod; fica para o esforço de migração (junto com healthcheck + secrets do Swarm). Sem mudança de código nesta rodada.
...
- **Severidade:** P3
- **Status:** Corrigido (2026-07-03)
- **Verificação:** Novo `src/lib/reports/export-limit.ts` (`MAX_EXPORT_ROWS = 10_000`, espelhando o `MAX_REPORT_ROWS` do hub de BI, + `truncationNotice()`). Os três `findMany` de export (`admin/referrals/commissions/export`, `admin/referrals/payouts/export`, `painel/financeiro/export-csv`) ganharam `take: MAX_EXPORT_ROWS`; ao atingir o teto, o CSV recebe uma linha-comentário final `# TRUNCADO … filtre por período`. Teste `src/lib/reports/export-limit.test.ts` (2). Portão verde: typecheck 0 · lint 0 erros (1 warn pré-existente) · build exit 0 · test 402 passed. Commit: 1e18005.
...
- **Severidade:** P3
- **Status:** Corrigido (2026-07-03)
- **Verificação:** O loop `for...of await` sequencial dos dois endpoints (`admin/catalogo/bulk`, `painel/cursos/bulk`) virou concorrência limitada via novo helper `src/lib/concurrency.ts` `runInChunks(items, 10, fn)` — updates INDIVIDUAIS concorrentes em lotes de 10 (`Promise.allSettled` por lote). **NÃO** reintroduz `$transaction([...map])` (padrão proibido no repo, 019a253/DB-004) — o proibido é o lote transacional dinâmico, não updates paralelos; `size=10` limita o uso do pool. O relatório `failed[]` por linha é preservado (allSettled: cada linha sucede/falha isolada, sem abortar as demais). Corta o wall-time de um lote grande (500 admin/200 painel) ~10×. Teste `src/lib/concurrency.test.ts` (4): ordem preservada, concorrência ≤ size, falha isolada não aborta, `size<1` lança. Portão verde: typecheck 0 · lint 0 erros (1 warn pré-existente) · build exit 0 · test 406 passed. Commit: 181b2a2.
