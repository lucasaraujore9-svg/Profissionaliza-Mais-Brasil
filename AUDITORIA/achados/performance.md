...
- **Verificação:** Os 4 crons ganharam teto por execução com progresso garantido (mantido o IO serial de propósito — rate limit EA/LMS/Asaas). Estratégia por tipo de progresso:
  ...
  Teste `src/app/api/cron/reconcile-tenant-payments/route.test.ts` (2): cursor lido→injetado no where, lote cheio avança o cursor, lote parcial zera. Portão verde: typecheck 0 · lint 0 erros (1 warn pré-existente) · build exit 0 · test 395 passed. Commit: <PENDENTE>. ⚠️MIGRAÇÃO VPS: o fix definitivo é BullMQ (job por item) — o cap/cursor é a ponte segura até lá.
