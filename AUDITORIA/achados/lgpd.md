...
- **Severidade:** P2
- **Status:** Corrigido _(2026-07-03, junto de OBS-008 — commit da rodada observabilidade. Nova função `src/lib/webhooks/redact-payload.ts` (`redactWebhookPayload`) mascara CPF/CNPJ/RG/e-mail/telefone e trunca texto livre (`body` do LMS) preservando ids/status/valores; aplicada nos 3 webhooks (`asaas` ×2, `mercadopago`, `lms`) antes do `prisma.webhookLog.create`. O cron `cleanup-webhook-logs` passou a REDIGIR (não apagar) o payload de `processed:false` com > 180 dias, cobrindo a PII histórica de webhooks falhos. Testes em `redact-payload.test.ts`. Retenção de 90d (processed:true) mantida — documentar no RoPA segue como pendência LGPD-004.)_
