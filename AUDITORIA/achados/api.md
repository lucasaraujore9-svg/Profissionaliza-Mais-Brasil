...
- **Severidade:** P2
- **Status:** ✅ Corrigido (2026-06-20) — adicionados schemas Zod de **envelope** em `src/lib/home/sections.ts` (`createSectionSchema`, `updateSectionSchema`, `reorderSectionsSchema`; `sectionKindSchema = z.enum(SECTION_KINDS)`) e plugados via `safeParse` em `src/lib/home/api.ts` (`createSection`/`updateSection`/`reorderSections`), substituindo os `typeof`/`Array.isArray` ad-hoc. Cobre os 6 handlers (admin + painel delegam às mesmas funções). A validação granular por kind (`validateSectionPayload`, com regras de negócio) permanece intacta. Mantém o mesmo contrato (400 em envelope inválido). Teste novo: `src/lib/home/sections.schema.test.ts` (17 casos: kind desconhecido, config malformada, enabled não-boolean, position negativa/não-finita, patch vazio, order não-array). Portão: tsc/lint/test (133) verdes; build delegado ao CI.
