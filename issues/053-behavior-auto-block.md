...
Implementar bloqueio automático de alunos por inadimplência. Quando Tenant entra OVERDUE: check billing_mode, se AUTO bloqueia na EA + DB, se MANUAL notifica revendedor. Quando pagamento resolvido: desbloqueia.

## Componentes Envolvidos
- Webhook Asaas PAYMENT_OVERDUE dispara bloqueio
- Webhook Asaas PAYMENT_RECEIVED dispara desbloqueio
- lib/escola-avancada usuarios/editar { status: bloqueado }
- Email notificação revendedor (modo MANUAL)

## Comportamentos
- `block-on-overdue-auto` — Tenant billing_mode=AUTO → bloqueia na EA
...
- `update-student-status-ea` — POST EA usuarios/editar { status: bloqueado/ativo }
...
- [ ] Se AUTO: POST EA usuarios/editar { aluno: ea_student_id, status: bloqueado }
...
- [ ] POST EA usuarios/editar { aluno: ea_student_id, status: ativo }
